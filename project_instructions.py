...
def render_subproject(*, name: str, owns: "list[str]", contracts: "list[str]",
    ...
    The ownership gate is named, not explained. `operator ownership check` is
    already a rule in the root block; what is missing there, and only here, is
    which paths this branch may touch.
    """
    ...
    body += (
        "\n"
        "`operator ownership check` refuses a branch that changed anything "
        "else.\n"
        "\n"
    ...
    if spent > SUBPROJECT_WORD_BUDGET:
        raise InstructionsError(
            f"the {name!r} subproject block is {spent} words, over the "
            f"{SUBPROJECT_WORD_BUDGET}-word budget by "
            f"{spent - SUBPROJECT_WORD_BUDGET}. A subproject file carries "
            "resolved facts and no rules (FR-9); if it needs prose, the prose "
            "belongs in the root file or a skill.")
    return block
...
def compose(existing: "str | None", managed: str) -> str:
    ...
    A file created from nothing is seeded with :data:`VALIDATION_STUB` below
    the block. That is where a project's build, test and lint commands live
    (D11), and it is written *once*: the seed only happens when there is no
    file, so a project that deletes the section is not given it back on the
    next regeneration. Handing it back would be the same fight over the same
    three lines that keeping the commands out of the block exists to end.
    """
